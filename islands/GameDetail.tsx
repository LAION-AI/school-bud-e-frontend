...
export default function GameDetail({ id }) {
    const [game, setGame] = useState<SavedGame>({ code: '', id: '', timestamp: '', totalPoints: 0 });
    ...
    useEffect(async () => {
        try {
            ...
            console.log(data)
            const game = data.filter((game) => game.id === id)[0];
            console.log(game)
            setGame(game)
        } catch (error) {
                                ...
                                onClick={async () => {
                                    ...
                                    try {
                                        const response = await fetch(`/api/game`, {
                                            ...
                                            body: JSON.stringify({ id: game.id, name: editedName })
                                        });
                                        const data = await response.json();
                                        if (data.success) {
                                            ...
                                            alert('Failed to update game name: ' + data.error);
                                        }
                                    } catch (error) {
                            ...
                            if (isEditingCode) {
                                try {
                                    const response = fetch('/api/game', {
                                        method: 'PUT',
                                        headers: { 'Content-Type': 'application/json' },
                                        body: JSON.stringify({ id: game.id, code: editedCode })
                                    });
